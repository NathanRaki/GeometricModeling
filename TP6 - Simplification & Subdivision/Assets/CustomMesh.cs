...
public class CustomMesh : MonoBehaviour
{
    ...
    void export()
    {
        ...
        File.WriteAllLines("MeshDst/" + file, lines.ToArray());
    }
    ...
    void simplifyMesh(int r)
    {
        ...
        // Check if cell contains vertices and add them if so
        foreach (Vector3 vertex in vertices)
        {
        ...
        foreach (Cell cell in cells)
        {
            ...
            cell.mean = new Vector3(0.0f, 0.0f, 0.0f);
        ...
        for (int i = 0; i < triangles.Count; i += 3)
        {
            ...
            // Get new vertices and break if more than 1 is in the cell
            bool skip = false;
            foreach (Cell cell in cells)
            {
                if (cell.vertices.Contains(p1)) { p1 = cell.mean; }
                if (cell.vertices.Contains(p2)) { p2 = cell.mean; }
                if (cell.vertices.Contains(p3)) { p3 = cell.mean; }
                if (p1 == p2 || p1 == p3 || p2 == p3) { skip = true; break; }
            }
            if (skip) { continue; }

            // Updating the new vertices and triangles lists
            int p1_index = new_vertices.IndexOf(p1);
            int p2_index = new_vertices.IndexOf(p2);
            int p3_index = new_vertices.IndexOf(p3);
            if (p1_index != -1) { new_triangles.Add(p1_index); }
            else
            {
                new_vertices.Add(p1);
                new_triangles.Add(new_vertices.Count - 1);
            }
            if (p2_index != -1) { new_triangles.Add(p2_index); }
            else
            {
                new_vertices.Add(p2);
                new_triangles.Add(new_vertices.Count - 1);
            }
            if (p3_index != -1) { new_triangles.Add(p3_index); }
            else
            {
                new_vertices.Add(p3);
                new_triangles.Add(new_vertices.Count - 1);
    ...
    void subdivideMesh(int r)
    {
        for (int j = 0; j < r; j++)
        {
            ...
            for (int i = 0; i < triangles.Count; i += 3)
            {
                ...
                Vector3 p4 = (p1 + p2) / 2.0f;
                Vector3 p5 = (p2 + p3) / 2.0f;
                Vector3 p6 = (p1 + p3) / 2.0f;

                int p1_index = new_vertices.IndexOf(p1);
                int p2_index = new_vertices.IndexOf(p2);
                int p3_index = new_vertices.IndexOf(p3);
                int p4_index = new_vertices.IndexOf(p4);
                int p5_index = new_vertices.IndexOf(p5);
                int p6_index = new_vertices.IndexOf(p6);

                // Triangle 1
                if (p1_index != -1) { new_triangles.Add(p1_index); }
                else { new_vertices.Add(p1); new_triangles.Add(new_vertices.Count - 1); }
                if (p4_index != -1) { new_triangles.Add(p4_index); }
                else { new_vertices.Add(p4); new_triangles.Add(new_vertices.Count - 1); }
                if (p6_index != -1) { new_triangles.Add(p6_index); }
                else { new_vertices.Add(p6); new_triangles.Add(new_vertices.Count - 1); }
                // Triangle 2
                if (p4_index != -1) { new_triangles.Add(p4_index); }
                else { new_vertices.Add(p4); new_triangles.Add(new_vertices.Count - 1); }
                if (p2_index != -1) { new_triangles.Add(p2_index); }
                else { new_vertices.Add(p2); new_triangles.Add(new_vertices.Count - 1); }
                if (p5_index != -1) { new_triangles.Add(p5_index); }
                else { new_vertices.Add(p5); new_triangles.Add(new_vertices.Count - 1); }
                // Triangle 3
                if (p6_index != -1) { new_triangles.Add(p6_index); }
                else { new_vertices.Add(p6); new_triangles.Add(new_vertices.Count - 1); }
                if (p5_index != -1) { new_triangles.Add(p5_index); }
                ...
                if (p3_index != -1) { new_triangles.Add(p3_index); }
                else { new_vertices.Add(p3); new_triangles.Add(new_vertices.Count - 1); }
                // Triangle 4
                if (p4_index != -1) { new_triangles.Add(p4_index); }
                else { new_vertices.Add(p4); new_triangles.Add(new_vertices.Count - 1); }
                if (p5_index != -1) { new_triangles.Add(p5_index); }
    ...
    {
        Mesh msh = new Mesh();
        loadOFF("MeshSrc/" + file);
        center();
        normalize();
        if (simplify) { simplifyMesh(simplification); }
        if (subdivide) { subdivideMesh(subdivision); }
        generateNormals();
        export();
